...
class SkillRuntimeBoundary(unittest.TestCase):
    def test_p2b_mutation_fence_is_owned_and_intent_surfaces_do_not_claim_receipt(self):
        ...
        lifecycle = (ROOT / "desktop/src-tauri/src/commands/runtime/lifecycle.rs").read_text()
        ...
        settings_intent = lifecycle.split(
            '} else {\n            let changed = config::update_result(&paths.config_dir', 1
        )[1].split("\n        }\n    })", 1)[0]
        self.assertIn('if changed { "committed" } else { "no_change" }', settings_intent)
