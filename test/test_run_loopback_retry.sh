...
out_a="$(cd "$ROOT" && CSSWITCH_LOOPBACK_TEST_FIXTURE=always-fail bash test/run-loopback.sh 2>&1)"; rc_a=$?
...
out_b="$(cd "$ROOT" && CSSWITCH_LOOPBACK_TEST_FIXTURE=pass-on-third bash test/run-loopback.sh 2>&1)"; rc_b=$?
