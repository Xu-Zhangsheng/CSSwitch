...
| 第三方运行包络 | 隔离 HOME、持久 data-dir、runtime identity、启动/停止/恢复 | 否：这是 CSSwitch 第三方模式责任 | `CSSWITCH-RUNTIME` | `必须托管` | 否 | `SOURCE`、`TEST` | final artifact 与 installed-live 连续性；当前 ambient environment allowlist 缺口 |
...
| Updater | Science check/apply/update/rollback | 是：Science updater 与最低支持版本 | `SCIENCE-NATIVE → SCIENCE-EXTERNAL`；候选采用另走 `CSSWITCH-RUNTIME` | `原生保留`；第三方运行固定 `--no-auto-update`，启动时只采用受校验候选 | 是：CSSwitch 不提供通用 check/apply/download UI | `SOURCE`、`TEST`、`PACKAGE-STATIC` | 当前 installed identity、候选采用、rollback 与 public release |
