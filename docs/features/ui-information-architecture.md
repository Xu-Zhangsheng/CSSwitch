...
适用范围：v0.8.0 起公开的模型连接、Skill & MCP、状态与设置四页 Desktop UI。本文只规定页面职责、信息层级和能力边界；具体版本已发布行为仍以各功能合同与发布证据为准。
...
- 后续 UI 变更合并前仍需在 Tauri App 复核模型连接、Codex 设置、Codex 动态目录、深色主题与最小窗口；浏览器 mock 不能替代真实 App 验收。

## 暂不扩展

- 不把 `auth.json`、`config.toml` 或完整 endpoint 放进主流程。
- 当前不提供 Skill 删除、detach、load 验证或完整生命周期管理。
- 当前不提供 MCP 列表、新建、编辑、启停或 attach。
