...
| in-flight runtime transaction | `Config.runtime_transaction` / `RuntimeTransactionRecord` | 持久；one-click、history recovery 与 healthy-reopen Gateway intent 是 typed V2 originator，interrupted-Gateway recovery 只 exact-CAS 推进已存在的 profile-switch record；profile-switch V1 只保留兼容读取，当前 V2 originator 仅限 healthy reopen 发布 `StartFormalGateway` |
...
- 当前产品没有由 `set_active_profile` 触发的运行中 profile-switch transaction；该命令只提交 selection，下一次一键开始按新的 active profile 重新走完整启动与补偿链。已删除无 production caller 的 `set_active_profile_txn` writer 及其 rollback/journal helpers。唯一当前 profile-switch originator 是 healthy reopen 的 bounded Gateway intent：它在最终 admission 下发布 V2 `StartFormalGateway`，不重启 Science，也不恢复已删除的通用 profile-switch coordinator；recovery 只 exact-CAS 推进这个既有 identity。
