...
- `stop_all`、`set_mode`、teardown `set_settings`、native exit、downgrade cleanup 与 cold one-click prior stop 已锁外等待并使用各自的 process-local owner/CAS publication；cold 路径额外保留 durable prior-stop intent/outcome。history、DB recovery、managed Science DB restart、compensation 与 replay stop 仍按各自事务与恢复合同保留现有边界，是否继续拆分必须逐项重新定界；
