...
- `stop_all`、`set_mode`、teardown `set_settings`、native exit 与 downgrade cleanup 已锁外等待并共享 process-local owner/CAS publication；transaction-scoped one-click/history/recovery/compensation stop 仍按各自事务与恢复合同保留现有边界，是否继续拆分必须逐项重新定界；
