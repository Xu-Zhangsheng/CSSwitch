...
- 本地目标分支：`next`
- 最后复核工程 HEAD：`6ac840b94fe0c4f958ef908abad9c70cb7ff91db`
- 相对前一工程基线：`b7605bbce41868fb91f77b09fb1dc686a7a52fb6`
- 当前已完成切片：`sandbox_session` 第一层目录化拆分
- 下一阶段：先评估 `sandbox_session` 第二层收口，再决定是否进入 Gateway `server.rs`
...
先对 `sandbox_session` 做只读的第二层收口评估：

- `transaction_tests.rs` 是否应按测试场景拆分，同时保持或显式迁移 test identity；
