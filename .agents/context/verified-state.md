...
| Current exact artifact | `PASS(scope=exact-artifact)`：`c678b1b` 新构建 `CSSwitch Test.app` 0.8.4；bundle `com.csswitch.test`，Desktop/Gateway/resources/Info.plist、hash、compiled Acceptance marker 与 fresh empty-HOME packaged Gateway `codex-auth status` 均受限通过。详见[2026-08-22 exact artifact evidence](../../docs/evidence/investigations/2026-08-22-csswitch-c678b1b-exact-artifact.md)。证据提交的 pre-evidence docs-only baseline/parent 为 `434cfe3`；该 docs-only descendant 不是 producing source |
| Guard-managed Acceptance Desktop entry | `PASS(scope=RM-42 Desktop production entry)`：同一 `c678b1b` exact `CSSwitch Test.app` 的 packaged `Contents/MacOS/desktop` 在新的 guard-managed HOME 中实际运行；它只使用 `$HOME/.csswitch-acceptance`、保留动态 Gateway / Science 端口且 Codex 实验开关关闭。运行中与停止后 `8765` 基线均不变，最终 `assert-stopped` 通过。详见[2026-08-22 Desktop isolation evidence](../../docs/evidence/investigations/2026-08-22-csswitch-c678b1b-isolated-desktop-entry.md) |
| Gateway / Science production wiring | `NOT-RUN`；本轮空 profile fixture 没有执行 one-click 或启动 Gateway / Science，Desktop entry PASS 不外推为完整 isolated-live |
