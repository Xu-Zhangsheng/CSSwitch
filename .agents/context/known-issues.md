...
- **旧 Skill Manager negative refactor（Phase 1 historical source closure）**：`a9efdf74d2e1e6f3e49644c1cf3b0d36648734cc` 从 `da55374` 基线删除未注册、未编译的 `desktop/src-tauri/src/commands/skills.rs`（2,052 行）与 `desktop/src-tauri/src/skill_manager/`（13,485 行），并把 `test_skill_runtime_boundary.py` 的假 production path 替换为真实注册的 `commands/skill_install.rs`；current runtime inventory、S3 ChangeRecord 与仅绑定这两个精确 orphan 路径的 fail-closed deletion mapping 同步对齐。Skill boundary 16/16、quality kernel 16/16、runtime mutation inventory 5/5、quality metadata / Phase 0 target impact、Desktop clippy `-D warnings` 与双 workspace fmt 均为 `PASS`；PY offline 为 30 passed / 2 个 loopback `ENV-BLOCKED` skipped。完整 Desktop 首轮 608 项只因 canonical 前置 Gateway binary 尚未构建而得到 566 passed / 1 precondition failure / 41 approved ignored；按 canonical 顺序构建 Gateway 后，该唯一失败测试原样重跑 1/1 `PASS`。在 deletion/add-copy、累计 diff 与 delete/re-add 三轮 HIGH 发现均修复后，第四次 fresh formal independent clean-context review（`clean-context=YES`）为 `PASS`（`BLOCK/HIGH/MEDIUM/LOW=0/0/0/0`）。当时 clean detached exact commit 的 canonical gate output root 为 `/private/tmp/csg.wXYLBo`：run `eb2ba23c9347a733eeb620343e754b99` 15/15 suites `PASS`、runner exit 0，completion-seal SHA-256 为 `2a834ae086993f0a7a334d9e78e34fdd6759d364b23b6fee3e9410ca2a876343`；该阶段临时 evidence root 已在验收后按授权清理，故本条只记录当时已递归校验并接受的 exact-commit source closure，不把已删除 root 冒充为当前可重跑 evidence，也不把 current HEAD 写为当前 `SOURCE-GREEN`。artifact、installed、isolated-live / authorized live、provider、SSH、Skill/MCP runtime、signing、notarization 与 release 均为 `NOT-RUN`。
- **Skill / MCP / Plugin Phase 2 inspect-only package adapters historical source closure**：多包格式的 component-wise compatibility、Agent / deterministic host / Science ownership、inspect-plan-confirm-apply、effect ledger、MCP revision/profile/auth 与演进/验收不变量已在[扩展控制面架构合同](../../docs/architecture/skill-mcp-plugin-control-plane.md)冻结。`0c5fad7bf325de6a26743fa93fbad0e59f53a5c7` 为 already-fetched archive bytes 与 caller-asserted 40-hex source claim 增加非执行、in-memory inspection adapter：确定性 component graph / report 标示 provenance unverified，支持外的 YAML 返回 typed `Partial`、Plugin / unknown manifests 为 `Unsupported`，不接入 install / apply、Tauri、Gateway、Agent、Science、MCP process、network 或 credential caller；Gateway / Desktop consumer lockfiles 和质量元数据均纳入同一 ChangeRecord。fresh independent clean-context review 为 `PASS`（`BLOCK/HIGH/MEDIUM/LOW=0/0/0/0`）。当时 canonical gate 的 exact candidate 是 `e2a3b60778880e3738e6d031b4e1300e2dffddc5`，output root 为 `/private/tmp/p2e.58DYcK`，run `566004c874b7f2c80b451a365d7d61a6` 为 15/15 suites `PASS`，executed/passed/ignored/failed/not-run 为 `1484/1439/45/0/0`；其 gated tree 与随后显式 staging 的提交 `0c5fad7bf325de6a26743fa93fbad0e59f53a5c7` 一致。该阶段临时 evidence root 已在验收后按授权清理，且没有保留可独立复核的 completion-seal digest；本条因此只记录当时已接受的 gated-tree source closure，不把已删除 root 冒充为当前可重跑 evidence，也不把 current HEAD 写为当前 `SOURCE-GREEN`。exact artifact、installed、Skill/MCP runtime、real Science/provider、signing、notarization 与 release 均为 `NOT-RUN`。现有 v1 bridge 行为不变；operation ledger、Science attachment、MCP process/profile/auth handling 及 inspect-plan-confirm-apply 的后续能力仍不由本项建立。
## 当前证据缺口
