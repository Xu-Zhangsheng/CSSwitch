...
fn o1_e3_compensation_replay_has_one_durable_pre_auth_owner() {
    ...
    assert!(
        replay_source.contains("ssh_stub_transaction")
            ...
            && replay_source.contains("runtime_environment_fingerprint_changed")
            && compensation_source.contains("runtime_environment_fingerprint_changed")
            && !compensation_source.contains("prior.runtime != failure.rollback.launch_runtime")
